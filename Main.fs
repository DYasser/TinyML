...
let interpret_expr tenv venv e =
    ...
    #endif
    //let t = Typing.typecheck_expr tenv e  //typecheck
    let t,s = Typing.typeinfer_expr tenv e  //typeinfer
    #if DEBUG
    printfn "type :\t%s" (pretty_ty t)
    //
    printfn "subst:\t%s" (pretty_subst s)   //typeinfer
    #endif
        ...
        trap <| fun () ->
            ...
            let x, (t, v) =
                ...
                | IBinding (_, x, _, _ as b) ->
                    ...
                    //tenv <- (x, t) :: tenv            //typecheck
                    tenv <- (x, Forall([],t)) :: tenv   //typeinfer
                    venv <- (x, v) :: venv
                    x, (t, v)
